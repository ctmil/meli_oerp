...
class mercadolibre_category_attribute(models.Model):
    ...
    def fix_attribute_create_variant( self ):
        ...
        # misma lista negra que meli_default_create_variant: la regla de "esto es
        # de variacion" vive en DOS lugares y las dos tienen que coincidir, o el
        # boton vuelve a promover lo que el importador ya dejo bien.
        if meli_attribute_is_never_variant( self.att_id ):
            meli_att_creates_variant = False
        ...
        if meli_att_creates_variant:
            ...
            # no habia forma de bajarlo desde la interfaz, y a mano Odoo lo
            # rechaza mientras el atributo este en uso.
            #
            ...
            # de las plantillas, y recien cuando no queda ninguna se cambia el
            # modo.
            for att in self.product_attributes:
                if (att.create_variant==default_create_variant):
                    for product_tmpl in att.product_tmpl_ids:
                        # FRENO: quitar la linea borra y recrea las variantes de
                        # esa plantilla. Si alguna tiene movimientos (stock o
                        # ventas) no se toca: se informa y se sigue con las demas.
                        if self._attribute_variants_have_movements( product_tmpl, att ):
                            _logger.warning(
                                "fix_attribute_create_variant: %s se saltea, "
                                "tiene variantes con movimientos (atributo %s)",
                        ...
                        for att_line in product_tmpl.attribute_line_ids:
                            if att_line.attribute_id.id==att.id:
                                try:
                                    ...
                                    _logger.error(
                                        "fix_attribute_create_variant: no se pudo quitar "
                                        "la linea de %s en %s: %s",
    ...
    def _attribute_variants_have_movements( self, product_tmpl, att ):
        ...
        movimientos de stock o lineas de venta. Sirve de freno: sin esto,
        bajar el atributo borraria variantes que ya operaron."""
                    ...
                    try:
                        ...
                        if (attrs.id):
                            if (company.mercadolibre_product_attribute_creation!='manual'):
                                ...
                                prod_att = {
                                    'name': att['name'],
                                    # se pasa el att_id de ML explicito: attrs_field solo lo
                                    # trae en la rama de create (linea ~449), y sin el la lista
                                    # negra GTIN/SELLER_SKU no puede decidir.
                                    'create_variant': self.env["product.attribute"].meli_default_create_variant(
                                        meli_attribute=dict(attrs_field, att_id=att['id'])),
                                    'meli_default_id_attribute': attrs[0].id,
