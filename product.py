...
class product_product(osv.osv):
    ...
    def product_meli_get_product( self, cr, uid, ids, context=None ):
        ...
        meli_fields = {
            ...
            'meli_title': response.content['title'],
            'meli_description': response.content['description'],
#            'meli_description_banner_id': ,
            'meli_category': response.content['category_id'],
            'meli_listing_type': response.content['listing_type_id'],
            'meli_buying_mode': response.content['buying_mode'],
            'meli_price': str(response.content['price']),
            'meli_price_fixed': True,
            'meli_currency': response.content['currency_id'],
            'meli_condition': response.content['condition'],
            'meli_available_quantity': response.content['available_quantity'],
            'meli_warranty': response.content['warranty'],
#            'meli_imagen_logo': fields.char(string='Imagen Logo', size=256),
#            'meli_imagen_id': fields.char(string='Imagen Id', size=256),
            'meli_imagen_link': response.content['thumbnail'],
#            'meli_multi_imagen_id': fields.char(string='Multi Imagen Ids', size=512),
            'meli_video': str(response.content['video_id'])
        }
