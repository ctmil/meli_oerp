...
### 24 sep 2026 — Mapeo de cancelaciones: la devolucion sobrevive a la cancelacion + el motivo se vuelve un dato [#607 PetMarkt, #530 R&D] (v19.0.26.142)
...
**Migracion SI.** `migrations/19.0.26.142/post-migrate.py` siembra `meli_cancel_reason_code` sobre el
...
### 23 sep 2026 — Landing de la tanda 26.127 + 26.129 sobre 26.133 [#492/#577, #409, #615] (v19.0.26.134)

**Qué entró.** Tres fixes que estaban listos en ramas `claude/*` sin mergear, más el #615 que ya
estaba en `origin/19.0`:
...
### 23 ago 2026 — `disable_cancel_warning_enabled = False`: la cancelacion FINGIA cancelar — v19.0.26.96 `[#494 Shoppy 502]`

Port del fix diagnosticado y **probado en 16.0** (server de TEST de Shoppy, 14/14 verdes, con control
de mutacion). Aca: **en esta version NO hay bug** (el core de Odoo 19 elimino el wizard y la clave `disable_cancel_warning`: `action_cancel()` llama siempre a `_action_cancel()`). Se porta igual por alineacion del source y porque la verificacion de estado vale igual.
