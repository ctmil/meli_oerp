...
### 24 sep 2026 — Mapeo de cancelaciones: la devolucion sobrevive a la cancelacion + el motivo se vuelve un dato [#607 PetMarkt, #530 R&D] (v18.0.26.142)
...
**Migracion SI.** `migrations/18.0.26.142/post-migrate.py` siembra `meli_cancel_reason_code` sobre el
...
### 23 ago 2026 — `disable_cancel_warning_enabled = False`: la cancelacion FINGIA cancelar — v18.0.26.96 `[#494 Shoppy 502]`
...
### 19 ago 2026 - fix(stock-diag): una respuesta de ERROR de ML se leia como "sin accion" -> corrida VERDE que no reviso nada (18.0.26.95) [#535 Score 361/499]
...
### 3 ago 2026 — `orders_query_iterate`: el truncamiento de la ventana deja rastro (decisión sobre `offset_next`) — v18.0.26.91 `[#427]`
...
### 23 jul 2026 — fix(publicación): foto fantasma `{'source':'None'}` en cada item ML (v18.0.26.86) `[flota/Koreautos]`
...
Branch `claude/fix-meli-logo-extra-image-18.0` (push automático). Merge a deploy + prod: NO (a confirmar).
...
**Fix:** `meli_oerp/models/company.py` (v18.0.26.82), branch paused del diagnóstico:
