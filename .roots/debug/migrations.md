...
## 24 sep 2026 - motivo de cancelacion estructurado (v18.0.26.142) [#607 PetMarkt]
...
- **Migracion `migrations/18.0.26.142/post-migrate.py` (post, idempotente):** siembra
...
## 28 jul 2026 - sello de movimientos de stock: create_date -> GREATEST(date, write_date, create_date) (v18.0.26.88) [OrgVit 475]
...
## 9 jul 2026 — BUG-009 `date_order` + BUG-007 cancelación en `update_order_status` (v18.0.26.68)
...
## 8 jul 2026 — Feature surtido multi-almacén (captura depósito ML) + fixes de migración (v18.0.26.66)
...
- **Feature (16≡17≡18≡19, byte-idéntico):** 2 campos Char nuevos en `mercadolibre.order_items`
  ...
  de datos: el ORM crea las columnas en el `-u` del bump 18.0.26.66. Anchors idénticos en las 4 versiones.
- **Adaptación v18.0:** `type='tree'`→`list` en `views/claims_view.xml` (Odoo 18/19 removió `tree`); quitado `numbercall` de `data/claims_cron.xml` (Odoo 17 removió el campo de `ir.cron`).
